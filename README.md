...
## 💕 자기 소개
프론트엔드 경험을 바탕으로 백엔드까지 확장하고 있는 풀스택 지향 웹 개발자입니다.  
React/Next.js 기반의 UI 구현과 상태 관리, 성능 최적화에 강점이 있으며,  
최근에는 Python/Django 기반으로 API, DB 모델링, 인증/권한, 서비스 로직까지 함께 다루고 있습니다.  
기능 구현에 그치지 않고, 사용자 경험과 운영 효율을 함께 개선하는 개발을 지향합니다.

## 핵심 역량
- 복잡한 요구사항을 사용자 흐름 기준으로 정리하고, 더 단순한 구조로 개선합니다.
- 문제를 재현하고 원인을 분석한 뒤, 대안을 비교해 현실적인 방식으로 해결합니다.
- 요구사항과 작업 맥락을 팀과 명확히 공유하며 협업 과정의 혼선을 줄이는 편입니다.
